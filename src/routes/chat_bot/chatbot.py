...
from src.infrastructure.chat_bot.chatbot_t import ChatBotT
...
@route.post('/v1/qa-chatbot2')
def qa_chatbot2(question: Question) -> Answer:
    chatbot = ChatBotT(index_name='tobecv', repo=PineconeRepository(),
                       embeddings_model=OpenAIEmbeddings(openai_api_key=credentials_provider.open_ai_api_key))
    answer = chatbot.query_question(question)
    return answer
